...
int main(int argc, char* argv[])
{
	int flags;
	VideoState *is;
	//av_strlcpy(input_filename, argv[1], sizeof(is->filename));
...
VideoState *stream_open(const char *filename, AVInputFormat *iformat)
{
	...
	printf("初始化videoq、audioq、subtitleq-----packet_queue_init\n");
...
void refresh_loop_wait_event(VideoState *is, SDL_Event *event) 
{
	...
	while (!SDL_PeepEvents(event, 1, SDL_GETEVENT, SDL_FIRSTEVENT, SDL_LASTEVENT)) 
	{
		...
		if (is->show_mode != SHOW_MODE_NONE && (!is->paused || is->force_refresh))
			video_refresh(is, &remaining_time);//视频的渲染
		SDL_PumpEvents();
	}
}

/* this thread gets the stream from the disk or the network */
//负责读取文件内容，将video和audio内容分离出来生成packet，将packet输出到packet队列中，
//包括Video Packet Queue和Audio Packet Queue（不考虑subtitle）
int read_thread(void *arg)
{
	...
	for (;;) {
		...
		{
			//printf("音频put-----packet_queue_put\n");
			packet_queue_put(&is->audioq, pkt);
		}
		else if (pkt->stream_index == is->video_stream && pkt_in_play_range
			&& !(is->video_st->disposition & AV_DISPOSITION_ATTACHED_PIC)) 
		{
			//printf("视频put-----packet_queue_put\n");
...
//负责读取Audio Packets Queue队列，将audio packet解码得到Audio Frame，
//将Audio Frame输出到Audio Frame Queue队列中。
...
//负责读取Video Packets Queue队列，将video packet解码得到Video Frame，
//将Video Frame输出到Video Frame Queue队列中。
...
{
	//由于frame_timer记下来视频从开始播放到当前帧播放的时间，所以暂停后，
	//必须要将暂停的时间（ is->video_current_pts_drift - is->video_current_pts）一起累加起来，并加上drift时间。
	if (is->paused)//视频暂停的话要更新时钟
	{
...
void video_refresh(void *opaque, double *remaining_time)
{
	...
	retry:
		...
		else 
		{
			...
			last_duration = vp_duration(is, lastvp, vp);// 将当前帧的pts减去上一帧的pts，得到中间时间差
			//既然要音视频同步，肯定要以视频或音频为参考标准，然后控制延时来保证音视频的同步，
			//这个函数就做这个事情了，下面会有分析，具体是如何做到的。
			delay = compute_target_delay(last_duration, is);
			//获取当前时间
			time = av_gettime_relative() / 1000000.0;
			//假如当前时间小于frame_timer + delay，也就是这帧改显示的时间超前，还没到，就显示图片不刷新
...
//因为音频是采样数据，有固定的采用周期并且依赖于主系统时钟，
//要调整音频的延时播放较难控制。所以实际场合中视频同步音频相比音频同步视频实现起来更容易。
...
//这个延时就是有上面的compute_target_delay函数计算出来的。
//计算延迟后，更新pts的代码
